...
class BiLSTMModel(nn.Module):
    def __init__(self, X_shape, HFF_shape, prompt_dict):
        ...
        self.prompt_dict = torch.tensor(prompt_dict).float().cuda()  # 将提示字典转换为浮点数张量并移到 GPU 上
        ...
        # 定义提示投影层
        self.pmp_proj1 = nn.Linear(512, 8)  # 线性层用于提示特征投影
    ...
    def forward(self, raw_input, feat_input, prompt, val=False):
        ...
        # 特征数据流的前向传播
        feat_out, _ = self.hff_lstm(feat_input.permute(0, 2, 1))
        ...
        combined_out = 0.3 * raw_out + (1-0.3) * feat_out
